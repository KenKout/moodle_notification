...
class MOODLE_NOTI:
    ...
    def login_moodle(self):
        if TYPE_SSO == 'CAS':
            ...
            resp = self.s.post(url, headers=self.headers, data=data, params=params)
...
def threading_get_course_detail(DATA_COURSE, moodle, courseid):
    DATA_COURSE.append({"id": courseid, "data": moodle.process_data(moodle.get_course_detail(courseid))})
    return DATA_COURSE
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(threading_get_course_detail, DATA_COURSE, moodle, course['id']) for course in moodle.total_course]
    ...
    while True:
        try:
            ...
            DATA_COURSE_NEW = []
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [executor.submit(threading_get_course_detail, DATA_COURSE_NEW, moodle, course['id']) for course in moodle.total_course]
                concurrent.futures.wait(futures)
                ...
                for data_old in DATA_COURSE:
                    for data_new in DATA_COURSE_NEW:
                        if data_new['id'] not in ID:
                            ...
                            send_notification(moodle.NON_CHANGE.copy()['added'].append({'name': data_new['id'], 'url': f'{URL_LOGIN}/course/view.php?id={data_new["id"]}'}))
